...
if ( [ ${#APCLOCK_ON} -ne 4 ] ) || ( [ ${#APCLOCK_OFF} -ne 4 ] ); then
	logger -s -t "$SCRIPTNAME" -p 5 "client_clock_on and/or client_clock_off not set correctly to hhmm format. aborting."
	exit 1
fi
if [ "$APCLOCK_ON" = "$APCLOCK_OFF" ]; then
	logger -s -t "$SCRIPTNAME" -p 5 "client_clock_on and client_clock_off are set to the same value. aborting."
	exit 1
fi
