...
function get_director_url()
  ...
  return uci:get('tecff', 'director', 'url')
end

-- Returns switch-after time from UCI as UNIX epoch
function get_switch_time()
  local uci = require('simple-uci').cursor()
  local switch_time = uci:get('tecff', 'director', 'switch_after')
...
function get_offline_treshold()
  local uci = require('simple-uci').cursor()
  local offline_treshold = uci:get('tecff', 'director', 'switch_after_offline')
...
function is_offline_treshold_reached()
  ...
  if not path_exists("/tmp/tecff_director_gw_unreach") then
    return false
  end
  ...
  local f = io.open("/tmp/tecff_director_gw_unreach")
  if not f then
    return false
  end
...
function is_enabled_uci()
  ...
  return uci:get_bool("tecff", "director", "enabled")
end
